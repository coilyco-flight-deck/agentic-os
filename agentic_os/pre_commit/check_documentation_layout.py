...
The single source of truth for Markdown caps, so prose points here rather than
restating a number. Markdown may live only at the repo root under a small
allow-list, in flat docs/*.md or guides/*.md, in a skill folder, under an
examples/ tree, or as a co-located module README.md in one of two capped shapes.
docs/ is reference and capped short. guides/ is narrative, has no count cap, and
only ratified flagship repos open one. Caps: docs/documentation-bands.md.
"""
