#!/usr/bin/env python3
"""Assert a repo references the seed skills its code requires.

A repo containing code in language X should reference the matching
`coding-<lang>` seed skill, plus every always-on baseline, so that context is
present for ingestion. Languages are detected by file extension against the seed
table shipped in the package.
...
def referenced_skills(doc_texts: Iterable[str], skills: Iterable[str]) -> set[str]:
    """Of `skills`, those whose delivery or source path appears in docs."""
    blob = "\n".join(doc_texts)
    return {
        skill
        for skill in skills
        if canonical_ref(skill) in blob or source_ref(skill) in blob
    }
