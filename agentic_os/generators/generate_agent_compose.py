...
for files named AGENTS.COMPOSE.md. AGENTS.COMPOSE.md is the
...
Scope filtering selects which sources compose on this machine. The
...
Alongside the composed context, the composer emits a machine-readable
mount-eligibility manifest at
~/.config/agent-compose/mount-eligibility.json: for each harness, the set of
repos it may mount on this host, resolved to ~/projects/<org>/<repo> paths. The
set reuses the same per-harness source selection (scope + harness filtering) the
composer already computes, unioned with an always-on default mount set, so ward
reads the mount decision instead of re-deriving it - one source of truth.

Usage:
    agent-compose            # compose + (re)point symlinks + write manifest
...
from agentic_os.config import projects_root
...
# Mount-eligibility manifest: the composer's scope/harness selection as
# the repos each harness may mount. See docs/mount-eligibility-manifest.md.
MOUNT_MANIFEST_PATH = CONFIG_DIR / "mount-eligibility.json"
MANIFEST_BANNER = "generated by agent-compose from agent-compose.yaml; do not edit"

# Always-on default mount set: mounted regardless of task or harness.
# lore = agent knowledge, agentic-os = doctrine, aosk = operating context.
DEFAULT_MOUNT_SET = (
    "coilysiren/lore",
...
def manifest_path_for(composed_path: Path) -> Path:
    """The mount-eligibility manifest path sitting beside composed_path.

    Derived from composed_path (not the module constant) so a test driving run()
    against a tmp composed path gets its manifest in the same tmp dir, exactly
    as the symlink and stale-output logic key off composed_path.
    """
    return composed_path.with_name(MOUNT_MANIFEST_PATH.name)


def default_mount_paths(projects: Path) -> list[Path]:
    """Resolve the always-on default mount slugs to ~/projects/<org>/<repo>."""
    return [projects / slug for slug in DEFAULT_MOUNT_SET]


def repo_for_source(source: Path, projects: Path) -> Path | None:
    """Map a source file to the ~/projects/<org>/<repo> repo it lives in.

    Mount eligibility is per repo, not per source file: a source nested anywhere
    inside a repo still makes that whole repo mountable. The org and repo are the
    first two path components under the projects root, mirroring the
    ~/projects/<org>/<repo> workspace layout (agentic_os.config). A source that
    is not under the projects tree - an out-of-repo ~/.config/agent-compose
    doctrine file, the kind a root may legitimately point at - backs no mountable
    repo and returns None.
    """
    try:
        rel = source.resolve().relative_to(projects.resolve())
    except ValueError:
        return None
    if len(rel.parts) < 3:
        # Fewer than org/repo/file: the source sits above any repo root.
        return None
    return projects / rel.parts[0] / rel.parts[1]


def mount_eligibility(
    slices: dict[str, list[Path]], projects: Path
) -> dict[str, list[Path]]:
    """Resolve each harness's selected sources to the repos it may mount.

    The eligible set for a harness is the repos backing its scope- and
    harness-filtered sources, unioned with the always-on default mount set, then
    sorted and deduplicated. It reuses the composer's own per-harness selection
    (`slices`) so the mount decision cannot drift from the context-composition
    decision: ward reads this, it never re-judges scope.
    """
    defaults = default_mount_paths(projects)
    result: dict[str, list[Path]] = {}
    for harness, selected in slices.items():
        repos = set(defaults)
        for src in selected:
            repo = repo_for_source(src, projects)
            if repo is not None:
                repos.add(repo)
        result[harness] = sorted(repos, key=str)
    return result


def render_manifest(slices: dict[str, list[Path]], projects: Path) -> str:
    """Render the mount-eligibility manifest as deterministic JSON.

    Deterministic (sorted keys, no timestamps) so the drift check reproduces it
    byte-for-byte, exactly like compose(). `defaults` surfaces the always-on set
    for provenance; each `harnesses` entry is the complete mountable set (its
    eligible repos already unioned with the defaults) so ward reads one key and
    needs no second merge.
    """
    eligibility = mount_eligibility(slices, projects)
    payload = {
        "banner": MANIFEST_BANNER,
        "projects_root": str(projects),
        "defaults": [str(p) for p in default_mount_paths(projects)],
        "harnesses": {
            harness: [str(p) for p in repos]
            for harness, repos in sorted(eligibility.items())
        },
    }
...
def compose(sources: list[Path], overrides: dict[Path, Path] | None = None) -> str:
    ...
    Deterministic (no timestamps) so the drift check can reproduce it.
...
def run(config_path: Path, composed_path: Path, *, dry_run: bool = False) -> int:
    ...
    manifest_path = manifest_path_for(composed_path)
    manifest = render_manifest(slices, projects_root())

    if dry_run:
        # A true preview: emit "would write"/"would link" only on a real change,
        # so a converged host previews as a no-op for downstream changed_when.
    ...
    # Emit the manifest beside the composed output, writing only on real change
    # (mirroring the composed files) so the freshen hook stays silent on a no-op.
    if (
        manifest_path.read_text(encoding="utf-8")
        if manifest_path.exists()
        else None
        ...
        print(f"wrote   {manifest_path} (mount-eligibility manifest)")
    for harness, dst in sorted(load_points.items()):
...
def check(config_path: Path, composed_path: Path) -> int:
    """Verify composed_path matches a fresh compose. Exit 1 on drift.
    ...
    manifest_path = manifest_path_for(composed_path)
    expected_manifest = render_manifest(slices, projects_root())
    actual_manifest = (
        manifest_path.read_text(encoding="utf-8") if manifest_path.exists() else ""
    )
    if actual_manifest == expected_manifest:
        print(f"agent-compose: {manifest_path} in sync")
    else:
        drifted = True
        sys.stderr.write(
            f"agent-compose: drift - {manifest_path} (mount-eligibility manifest) "
            "is missing, stale, or hand-edited. Run `agent-compose` to regenerate.\n"
        )
        diff = difflib.unified_diff(
            actual_manifest.splitlines(),
            expected_manifest.splitlines(),
            fromfile=f"{manifest_path} (on disk)",
            tofile="expected (fresh compose)",
            lineterm="",
        )
        for line in list(diff)[:40]:
