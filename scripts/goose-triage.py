#!/usr/bin/env python3
"""goose-triage: real issue triage driven by a pluggable judgment engine.

Implements the tooling-issue-prioritization method with Python owning the
deterministic parts and a swappable engine owning the bounded per-issue
judgment. The engine is the local Goose+Qwen harness by default (`goose-json`),
but `--engine` / `$AOS_TRIAGE_ENGINE` selects a stronger judge - the bundled
Claude-CLI judge (`claude`) or any external command over the goose-json contract
(`command`) - so a re-triage reuses this audited scaffolding instead of being
...
from triage_engines import DEFAULT_ATTRIBUTION, ENGINE_NAMES, select_engine_from_env

# Live bindings the whole pipeline reads; main() rebinds both from the selected
# engine (goose-json default). Module globals: tests monkeypatch `ask`. See #271.
...
def render_comment(it: dict, today: str) -> str:
    """The marked verdict comment for one issue: tier + mode, each with Goose's
    reason. Marker-led so a re-run can find and edit this exact comment in place."""
    tier = it.get("tier", "P3")
    score = it.get("score")
    score_s = f" (score {score:.0f})" if isinstance(score, (int, float)) else ""
    reason = it.get("reason") or "no reason recorded"
    mode = it.get("mode", MODE_DEFAULT)
    mode_reason = it.get("mode_reason") or "no reason recorded"
    return "\n".join([
        COMMENT_MARKER,
        f"**Goose triage** - {today}",
        "",
        f"- **Tier:** `{tier}`{score_s} - {reason}",
        f"- **Mode:** `{mode}` - {mode_reason}",
        "",
        f"<sub>Auto-generated by `ward exec goose-triage` (judge: {ATTRIBUTION}), "
        "edited in place on each run. Labels are the conclusion; this is the why.</sub>",
    ])
...
def apply_comments(repo: str, result: dict) -> dict:
    ...
    for it in sorted(result["issues"], key=lambda d: d["num"]):
        ...
        if outcome.startswith("FAILED"):
            counts["failed"] += 1
            print(f"  comment #{it['num']}: {outcome[:100]}", file=sys.stderr)
        else:
            counts[outcome] += 1
            print(f"  comment #{it['num']}: {outcome}", file=sys.stderr)
    return counts
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--no-comment", dest="comment", action="store_false",
                    help="apply labels but skip the per-issue verdict comment")
    ap.add_argument("--engine", choices=ENGINE_NAMES, default=None,
                    help="judgment engine (default goose-json, or $AOS_TRIAGE_ENGINE): "
                         "'goose-json' local Goose+qwen, 'claude' the bundled Claude-CLI judge, "
                         "'command' an external judge via --engine-cmd")
    ap.add_argument("--engine-cmd", dest="engine_cmd", default=None,
                    help="for --engine command: the judge command, goose-json CLI contract "
                         "(--schema FILE --prompt-file FILE -> JSON). Or $AOS_TRIAGE_ENGINE_CMD")
    ap.add_argument("--engine-attribution", dest="engine_attribution", default=None,
                    help="override the judge name shown in the report and comment footer")
    ...
    global ask, ATTRIBUTION
    try:
        engine = select_engine_from_env(args.engine, args.engine_cmd, args.engine_attribution)
    ...
    if args.report_only:
        ...
        if args.comment:
            print(f"goose-triage: writing verdict comments to {repo} ...", file=sys.stderr)
            result["applied"]["comments"] = apply_comments(repo, result)
    ...
    failed = bool(ap_res) and (ap_res["tiers"]["failed"] or ap_res["modes"]["failed"]
                               or (ap_res.get("comments") or {}).get("failed", 0))
