...
class VadDaemon:
    ...
    def _commit(self):
        self.kbd.chord()
        time.sleep(self.commit_delay)  # let Wispr's paste land before submitting
        self.kbd.enter()
    ...
    def run(self):
        ...
        with sd.InputStream(
            ...
            log.info(
                "VAD daemon up on UDP %d (silence=%.1fs threshold=%.2f device=%s)",
                self.port,
...
def parse_args():
    ...
    p.add_argument("--commit-delay", type=float, default=0.15, help="pause between the Wispr toggle and Enter so the paste lands first")
