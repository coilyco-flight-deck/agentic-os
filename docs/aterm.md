...
**A window belongs to the bundle holding the binary that drew it.** Calling kitty where it lives credited every window to kitty, the name the Dock, the switcher, and Mission Control showed for every role. Each bundle links the terminal in beside its launcher and opens through it, so those surfaces read the role, and `LSUIElement` went with it because an accessory app cannot own the front window. A shell launch prefers the role's installed bundle too. A launch failing before a window opens has nothing to hold the error, so the wrapper alerts through `osascript`.
...
**A bundle is named for who answers.** The Dock shows `Vera // Systems Administrator`, the person and role rather than the harness. A POSIX filename cannot hold a slash, so the name is stored with ` :: `, which macOS renders as one, and only the directory carries it: the executable stays a plain `aterm-<role>`.
