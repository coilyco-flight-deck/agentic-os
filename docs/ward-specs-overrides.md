...
`model`, `reasoning-effort`, and `verbosity` overrides. Marker-bounded `name`
and `pronouns` fields identify the AOSH-selected role and harness seat. The
advisor role is a personal overlay concern, not part of the shipped product
role catalog.
...
`ward exec sync-role-seats` separately rewrites only the generated identity
region in each existing Ward agent block. AOSH owns the public-safe seat name
and pronouns. AOS owns the surrounding model, reasoning, verbosity, and
permission policy. A source seat without an existing Ward agent block fails
closed instead of creating a new executable harness.
