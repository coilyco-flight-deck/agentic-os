...
<!-- freshness: as-of=2026-07-09 decay-class=derived half-life=fast source="ward ops forgejo describe" -->

# ward ops forgejo - committed reference

**as-of:** 2026-07-09 - **source:** `ward ops forgejo describe` (v0.504.0) - **regenerate:** `ward exec gen-ward-ops-reference`
...
## ward ops forgejo workflow dispatch
...
## ward ops forgejo pr merge - merge a pull request (POST /repos/{owner}/{repo}/pulls/{index}/merge). Controlled director authority: ward-owned runs may land only when the issue thread says workflow: pull-request-and-merge, the final WARD-OUTCOME is done, and the review summary passed.
