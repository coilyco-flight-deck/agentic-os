...
The session already owns writable worktrees under `<session>/projects`, one per resident checkout, each on its own `aos/<harness>/<id>` branch. Linking the host projects root into the shadow home would give the canonical checkouts a second name inside the session. That name resolves past the worktrees to the shared trees on `main`.
...
Leaving the entry absent turns that into an immediate "no such file or directory" instead of a silent wrong answer.
...
Reaching the canonical tree on purpose is still possible through its absolute `/Users/<user>/projects` path. The guard removes the accident, not the capability.
