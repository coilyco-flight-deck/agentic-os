...
Opt-in composer (`agent-compose`) that synthesizes one canonical global context file at `~/.config/agent-compose/COMPOSED.md` from a declared set of sources, then points every harness's global load point (Claude Code, Codex, OpenClaw) at that single file by symlink. The three harnesses load byte-identical context with no content duplicated on disk. Sources are listed explicitly or discovered by walking declared roots for `AGENTS.COMPOSE.md` files, the disjoint always-global doctrine that no harness's own AGENTS.md/CLAUDE.md cascade loads. Scope tags pick what composes per machine: the machine declares `scopes`, each source declares its own `scopes` in frontmatter, and a source composes only where the two intersect, so one config is correct on every host. Activation is the presence of `~/.config/agent-compose/agent-compose.yaml`: with no config, the composer is a total no-op and every harness behaves exactly as it does without it. `setup.sh` runs it idempotently on each host.
