...
agentic-os dogfoods these actions in `.forgejo/workflows/`, using local `uses: ./actions/...` refs so the source repo never waits on its own mirror. Releasing is two-stage (ward#1117 / aos#469): `promote.yml` gates every main push and fast-forwards `release` when green; `release.yml` runs on `release` pushes under a no-cancel concurrency queue - gate again, publish the image family, tag last. The consumer pin is tag-derived (agentic-os#238). Major bumps stay hand-cut via `scripts/release.py`. `workflow_dispatch` re-fires either stage on a missed enqueue. Walkthrough: [docs/release.md](release.md).
