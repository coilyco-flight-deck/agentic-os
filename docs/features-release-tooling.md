...
agentic-os dogfoods these actions in `.forgejo/workflows/`, using local `uses: ./actions/...` refs so the source repo never waits on its own mirror. Releasing is two-stage (ward#1117 / aos#469): `promote.yml` gates every main push, publishes the image family, creates the tag and asset, then fast-forwards `release`; `release.yml` is the same manual retry path and never runs on push. The consumer pin is tag-derived (agentic-os#238). Major bumps stay hand-cut via `scripts/release.py`. `workflow_dispatch` re-fires either stage on a missed enqueue. Walkthrough: [docs/release.md](release.md).
