...
agentic-os dogfoods these actions in `.forgejo/workflows/`, using local `uses: ./actions/...` refs so the repo never waits on its mirror. Release is two-stage (ward#1117 / aos#469): `promote.yml` gates every main push, publishes images, creates the tag/asset, then fast-forwards `release`; `release.yml` is the manual retry path and never runs on push. PR retriggers need a real tracked diff. Consumer pin is tag-derived (agentic-os#238). Major bumps stay hand-cut via `scripts/release.py`. `workflow_dispatch` re-fires either stage on enqueue miss. Walkthrough: [docs/release.md](release.md).
