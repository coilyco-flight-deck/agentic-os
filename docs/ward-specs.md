...
* **`capabilities`** - a list of `provider/skill-dir` strings, read by `agentic-os-kai/scripts/pull-capabilities.py` to pull capability skills down into one leaf repo. Never part of Ward's documented schema, and declared only by `coilyco/galaxy-gen`. That list now lives in `capability_pull.repos` in AOSK's `categories.yaml`, since one script in one repo is the lowest layer that fully determines it.
