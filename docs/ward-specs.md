...
reinstate it. When a coilyco
fleet/guardfile/spec-lock value changes, change it **here in aos's `.ward/`** and
let a push republish the bundle. ward's tree is being neutralized (ward#503 step
4), after which ward carries no coilyco values and derives its whole shipped
...
corollary. The bundle is not fleet config every ward user melds, it is Kai's
single coilyco deployment, and aos is the one repo only Kai clones. External ward
users build neutral and never fetch it. The exception is stated in
[AGENTS.md](../AGENTS.md).
