...
1. **Fetch** open issues via one forgejo REST list call (bodies come back inline, no per-issue follow-up).
...
The forgejo list endpoint caps at 50 per page, so larger repos are fetched partially and the script warns the shape is over a partial set. Whole-backlog auto-pagination is tracked in ward#131.
