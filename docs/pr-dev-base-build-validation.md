...
In agentic-os#452, the dev-base publish path went red **after merge**: the
docker build only ran in the mainline publish flow, so no PR ever exercised it.
A broken Dockerfile, a stale pinned `ARG`, or a `WARD_VERSION` / swagger-lock
desync surfaced only after merge - and a red publish also starves the fleet of
a current `:latest`.

## One shared definition

The build/verify half is factored into the
[`actions/dev-base-build`](../actions/dev-base-build/action.yml) composite,
so the PR path and the main publish path share the same build script:

- `ci.yml`'s `build-dev-base` job runs it on every `pull_request` with
  `push: false` - build only.
- `promote.yml` and the `actions/publish-dev-base-tier` publish path keep
  using the same `scripts/dev-base-build.py` plan and build logic on `main`,
  with the registry token only in the publish path.
