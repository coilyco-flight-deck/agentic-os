...
`AOS_LAUNCH_SPEC=1` switches a native launch to spec mode, off by default until a canary passes. aos runs `agent-compose launch --spec-out <file> <role> <harness>`, which composes and projects and then exits without exec. aos then applies the spec's env and the runtime home (`HOME`, `USERPROFILE`, `CODEX_HOME`, `XDG_CONFIG_HOME`, and `CLAUDE_CONFIG_DIR` for claude), and for claude adds `--name <seat>` and `--settings`, rendered as above with the theme installed under the runtime home's `.claude/themes/`. Finally it execs the harness itself. A flag the caller passed wins. MCP role scoping has not moved yet, so spec mode runs claude and codex with the unscoped inventory until it does.
