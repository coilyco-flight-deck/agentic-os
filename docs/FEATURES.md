...
- [Goose issue triage](goose-triage.md) - `ward exec goose-triage` runs real issue triage with the local Goose + `qwen3-coder:30b` harness as the judgment engine: P0 regex net, Goose confirm, multi-pass numeric urgency scoring with a tie run-off, percentile cut into P0-P4, plus a second orthogonal **automation-mode** axis (`headless`/`interactive`/`consult`, the agent-autonomy ceiling an issue is cleared for, fail-closed to consult), then applies both as forgejo labels by default (`--report-only` to skip). Each judgment call routes through `ward exec goose-json` for a provider-enforced JSON-schema response from the local model (no regex scraping).
