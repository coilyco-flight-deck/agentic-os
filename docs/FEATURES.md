...
- [Test harnesses](test-harness.md) - one doc per agent (`test-harness-<agent>`) probing a harness+model pairing before trusting it with real work: what it claims about itself vs what it can actually do. Goose ships a wrapper script and ward verb (`ward exec goose-ask`).
- [Goose issue triage](goose-triage.md) - `ward exec goose-triage` runs issue triage with local Goose+`qwen3-coder:30b`: P0 regex net, Goose confirm, urgency run-off cut into P0-P4, plus an orthogonal **automation-mode** axis (`headless`/`interactive`/`consult`, fail-closed), applied as forgejo labels with a per-issue **verdict comment** giving Goose's reason per axis. Each judgment routes through `ward exec goose-json` for a provider-enforced JSON-schema reply, with failure-record instrumentation ([goose-failure-records.md](goose-failure-records.md)).
...
- [dev-base container image](dev-base-image.md) - the inner-loop toolchain (uv, pre-commit, node, go, aws cli, claude) published per release to the forgejo registry as `coilyco-flight-deck/agentic-os:<tag>`, multi-arch, pulled by tag in ward. Bakes a **public substrate seed** (bare mirrors of the image-tier reference repos at `/opt/substrate-seed`) so a ward container warms a cold gitcache with no network.
