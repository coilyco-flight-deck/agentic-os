...
- [Forgejo runner-token fetch overlay](forgejo-runner-token.md) - guarded runner registration-token minting through declarative fetch leaves instead of raw curl.
- [Role surface tiers](role-surface-tiers.md) - the director/ops runner-token split plus the guardfile ownership map for the bundle.
