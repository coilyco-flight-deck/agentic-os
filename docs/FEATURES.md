...
- [Agents and sessions](features-agents-sessions.md) - agent self-name, session pulse, and a project-local status-line repo-checkout tracker ([`.agentic-os/statusline.sh`](../.agentic-os/statusline.sh)) that names the `org/repo` clones on disk and flags ones that appeared since a baseline.
...
- [Goose issue triage](goose-triage.md) - `ward exec goose-triage` triages open issues with local Goose+`qwen3-coder:30b`: P0 net + confirm, urgency run-off cut into P0-P4, plus an **automation-mode** axis (`headless`/`interactive`/`consult`), applied as forgejo labels via `ward ops forgejo` (ward-kdl). Judgments route through `ward exec goose-json` ([goose-failure-records.md](goose-failure-records.md)).
