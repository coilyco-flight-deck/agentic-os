...
- [Goose test harness](goose-test-harness.md) - `ward exec goose-ask` probes the Goose agent with one-shot questions (single or a batch battery), strips the banner, times each call, and tees a raw transcript. Used to interrogate a Goose+model pairing before trusting it with real work.
