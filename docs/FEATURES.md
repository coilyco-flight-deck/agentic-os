...
- [Goose issue triage](goose-triage.md) - `ward exec goose-triage` runs issue triage with local Goose+`qwen3-coder:30b`: P0 regex net, Goose confirm, urgency scoring + run-off, percentile cut into P0-P4, plus an orthogonal **automation-mode** axis (`headless`/`interactive`/`consult`, fail-closed), then applies both as forgejo labels and upserts a per-issue **verdict comment** with Goose's reason per axis (`--report-only` skips writes, `--no-comment` it). Each judgment routes through `ward exec goose-json` for a provider-enforced JSON-schema reply, with per-`ask()` failure-record instrumentation ([goose-failure-records.md](goose-failure-records.md)).
