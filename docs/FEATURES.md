...
- [Goose issue triage](goose-triage.md) - `ward exec goose-triage` runs issue triage with local Goose + `qwen3-coder:30b` as the judgment engine: P0 regex net, Goose confirm, multi-pass urgency scoring with a run-off, percentile cut into P0-P4, plus an orthogonal **automation-mode** axis (`headless`/`interactive`/`consult`, the agent-autonomy ceiling, fail-closed), then applies both as forgejo labels and upserts a per-issue **verdict comment** with Goose's reason per axis (edited in place; `--report-only` skips writes, `--no-comment` the comment). Each judgment routes through `ward exec goose-json` for a provider-enforced JSON-schema reply.
