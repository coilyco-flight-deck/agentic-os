...
`release.yml` computes the next tag, then cuts the git/release tag and the
ward-specs bundle from `plan-release` alone - an image flake cannot cost the
release (aos#490). Then `publish-dev-base` runs
[`scripts/dev-base-build.py`](../scripts/dev-base-build.py) with bounded
per-tier push retries: a failed tier fails loudly without touching the tag,
and `:release` keeps pointing at the last good image set. There is no test
gate on `release`: promote.yml already ran the full suite on the exact sha
it fast-forwarded.
...
`actions/tag-bump` also has a compute-only mode: `plan-release` derives the
next semver, and the `release` job creates the tag.
