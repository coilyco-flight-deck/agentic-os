...
func TestLaunchPlanNamesOnlyClaudeSessionsTheCallerLeftUnnamed(t *testing.T) {
	for _, testCase := range []struct {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			carriesName := slices.Contains(plan.Child, stableSessionName)
...
func TestClearClaudeNamesASurvivor(t *testing.T) {
	reaper, _, notice := fakeProcesses(900, []processEntry{{PID: 10, PPID: 1, Command: "claude --name aterm"}})
	reaper.alive = func(int) bool { return true }
	if got := reaper.clearClaude(stableSessionName); got != 0 {
