...
func TestWrapChildPutsTheHarnessBehindVt(t *testing.T) {
	...
	got, wrapped := wrapChild(child, stableSessionName("Angie", "platform-eng"), true, stubLookPath, &bytes.Buffer{})
	trampoline := []string{"/stub/vt", "-S", "/bin/sh", "-c", renameThenRun, "sh", stableSessionName("Angie", "platform-eng"), "/stub/vt"}
...
func TestLaunchPlanWrapsInVibeTunnelUnlessOptedOut(t *testing.T) {
	for _, testCase := range []struct {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			out, err := runAterm(t, stubDeps(t, &spawns, true), append(args, "platform-eng", "claude")...)
			if err != nil {
...
func TestRunSessionRunsTheChildThroughVtNamedAndKeepsItsExitCode(t *testing.T) {
	...
	code := runSession(
		sessionOptions{
			VibeTunnel: true,
			Card:       sessionCard{Role: "platform-eng", Name: "Angie"},
	...
	if got := strings.TrimSpace(string(raw)); !strings.HasSuffix(got, "sh platform-eng-angie "+filepath.Join(dir, "vt")+" /bin/sh -c exit 7") {
		t.Fatalf("vt argv = %q", got)
	}
	// The name is set from inside the session, before the harness starts.
	if got, _ := os.ReadFile(titles); strings.TrimSpace(string(got)) != "title platform-eng-angie" {
