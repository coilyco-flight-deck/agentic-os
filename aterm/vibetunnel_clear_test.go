...
func TestClearTerminatesARunningSessionAndRemovesItsRecord(t *testing.T) {
	...
	dir := writeSessionRecord(t, control, "fwd_1_1", runningRecord(stableSessionName("Angie", "platform"), began))
	reaper, sent, _ := fakeReaper(control, func(sent []syscall.Signal) bool { return len(sent) == 0 }, began)
	if cleared := reaper.clear(stableSessionName("Angie", "platform")); cleared != 1 {
...
func TestClearLeavesAnotherRolesSessionRunning(t *testing.T) {
	control, began := t.TempDir(), time.Now()
	own := writeSessionRecord(t, control, "fwd_1_1", runningRecord(stableSessionName("Angie", "platform"), began))
	other := writeSessionRecord(t, control, "fwd_2_2", runningRecord(stableSessionName("Evie", "science"), began))
	reaper, sent, _ := fakeReaper(control, func(sent []syscall.Signal) bool { return len(sent) == 0 }, began)
	if cleared := reaper.clear(stableSessionName("Angie", "platform")); cleared != 1 {
		t.Fatalf("cleared = %d, want 1", cleared)
	}
...
func TestClearEscalatesToKillWhenTerminateIsIgnored(t *testing.T) {
	control, began := t.TempDir(), time.Now()
	writeSessionRecord(t, control, "fwd_1_1", runningRecord(stableSessionName("Angie", "platform"), began))
	...
	if reaper.clear(stableSessionName("Angie", "platform")) != 1 {
...
func TestClearKeepsARecordWhoseProcessWillNotEnd(t *testing.T) {
	...
	if cleared := reaper.clear(stableSessionName("Angie", "platform")); cleared != 0 {
...
func TestClearLeavesEveryOtherNameAlone(t *testing.T) {
	...
	if reaper.clear(stableSessionName("Angie", "platform")) != 0 || len(*sent) != 0 {
...
func TestClearRemovesAnEndedRecordWithoutSignalling(t *testing.T) {
	control, began := t.TempDir(), time.Now()
	ended := runningRecord(stableSessionName("Angie", "platform"), began)
	ended.Status = "exited"
	dir := writeSessionRecord(t, control, "fwd_1_1", ended)
	reaper, sent, _ := fakeReaper(control, func([]syscall.Signal) bool { return true }, began)
	if reaper.clear(stableSessionName("Angie", "platform")) != 1 || len(*sent) != 0 {
...
func TestClearIgnoresADirectoryTheServerWouldNeverName(t *testing.T) {
	control, began := t.TempDir(), time.Now()
	dir := writeSessionRecord(t, control, "not a session id", runningRecord(stableSessionName("Angie", "platform"), began))
	reaper, _, _ := fakeReaper(control, func([]syscall.Signal) bool { return false }, began)
	if reaper.clear(stableSessionName("Angie", "platform")) != 0 {
...
func TestClearEndsARealProcessThroughTheSystemReaper(t *testing.T) {
	...
	control := t.TempDir()
	record := runningRecord(stableSessionName("Angie", "platform"), began)
