...
func TestClearTerminatesARunningSessionAndRemovesItsRecord(t *testing.T) {
	...
	dir := writeSessionRecord(t, control, "fwd_1_1", runningRecord(stableSessionName("Angie", "eng-platform"), began))
	reaper, sent, _ := fakeReaper(control, func(sent []syscall.Signal) bool { return len(sent) == 0 }, began)
	if cleared := reaper.clear(stableSessionName("Angie", "eng-platform")); cleared != 1 {
...
func TestClearLeavesAnotherRolesSessionRunning(t *testing.T) {
	control, began := t.TempDir(), time.Now()
	own := writeSessionRecord(t, control, "fwd_1_1", runningRecord(stableSessionName("Angie", "eng-platform"), began))
...
func TestClearEscalatesToKillWhenTerminateIsIgnored(t *testing.T) {
	control, began := t.TempDir(), time.Now()
	writeSessionRecord(t, control, "fwd_1_1", runningRecord(stableSessionName("Angie", "eng-platform"), began))
	...
	if reaper.clear(stableSessionName("Angie", "eng-platform")) != 1 {
...
func TestClearKeepsARecordWhoseProcessWillNotEnd(t *testing.T) {
	...
	if cleared := reaper.clear(stableSessionName("Angie", "eng-platform")); cleared != 0 {
...
func TestClearLeavesEveryOtherNameAlone(t *testing.T) {
	...
	if reaper.clear(stableSessionName("Angie", "eng-platform")) != 0 || len(*sent) != 0 {
...
func TestClearRemovesAnEndedRecordWithoutSignalling(t *testing.T) {
	control, began := t.TempDir(), time.Now()
	ended := runningRecord(stableSessionName("Angie", "eng-platform"), began)
	ended.Status = "exited"
	dir := writeSessionRecord(t, control, "fwd_1_1", ended)
	reaper, sent, _ := fakeReaper(control, func([]syscall.Signal) bool { return true }, began)
	if reaper.clear(stableSessionName("Angie", "eng-platform")) != 1 || len(*sent) != 0 {
...
func TestClearIgnoresADirectoryTheServerWouldNeverName(t *testing.T) {
	control, began := t.TempDir(), time.Now()
	dir := writeSessionRecord(t, control, "not a session id", runningRecord(stableSessionName("Angie", "eng-platform"), began))
	reaper, _, _ := fakeReaper(control, func([]syscall.Signal) bool { return false }, began)
	if reaper.clear(stableSessionName("Angie", "eng-platform")) != 0 {
...
func TestClearEndsARealProcessThroughTheSystemReaper(t *testing.T) {
	...
	control := t.TempDir()
	record := runningRecord(stableSessionName("Angie", "eng-platform"), began)
