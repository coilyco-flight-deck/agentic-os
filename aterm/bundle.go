...
// name is the on-disk basename: the role alone, which the roster keeps unique.
// A slash from the roster would read as a path separator. agentic-os#8164
func (s bundleSpec) name() string {
	return strings.ReplaceAll(s.displayName(), "/", "-")
}

// displayName is the name the Dock draws, for the plist and alerts.
...
// seatLine keeps the person the name dropped, for Get Info.
func (s bundleSpec) seatLine() string {
	if person := strings.TrimSpace(s.Person); person != "" {
		return person + " // " + s.displayName()
	}
...
func newBundlesCommand(deps commandDeps) *cli.Command {
	return &cli.Command{
		...
		Flags: append(bundleBinaryFlags(), []cli.Flag{
			...
			&cli.BoolFlag{
				Name:  "prune",
				Usage: "after writing, remove the stale bundles this command wrote and this run does not",
			},
		}...),
...
func runBundles(ctx context.Context, deps commandDeps, cmd *cli.Command) error {
	...
	if cmd.Bool("prune") && cmd.Bool("check") {
		return withExit(exitUsage, fmt.Errorf("--check writes nothing, so it has nothing to prune"))
	}
	...
	if cmd.Bool("prune") {
		// Pruned only after every write landed, so a failed run removes nothing.
		if err := removeStale(stdout, plan.Stale); err != nil {
			return err
		}
		plan.Stale = nil
	}
...
// removeStale deletes each stale bundle, checking the marker again just before
// the removal, so an app this command did not write is never taken.
func removeStale(writer io.Writer, paths []string) error {
	for _, path := range paths {
		if _, ours := generatedRole(path); !ours {
			return fmt.Errorf("%q was not written by `aterm bundles`, so --prune leaves it", path)
		}
		if err := os.RemoveAll(path); err != nil {
			return fmt.Errorf("remove the stale bundle %q: %w", path, err)
		}
		if _, err := fmt.Fprintf(writer, "removed stale: %s\n", path); err != nil {
			return err
		}
	}
	return nil
