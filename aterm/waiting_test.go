...
func TestAFastCallNeverAnnouncesItself(t *testing.T) {
	notice := &bytes.Buffer{}
	got := whileWaiting(notice, []string{"aos", "_launch-agent", "dev-advocate"}, func() string {
		return "claude"
	})
	if got != "claude" {
...
func TestASlowCallNamesTheCommandItIsWaitingOn(t *testing.T) {
	notice := &bytes.Buffer{}
	command := []string{"aos", "_launch-agent", "dev-advocate"}
	...
	want := "aterm: waiting on `aos _launch-agent dev-advocate`"
