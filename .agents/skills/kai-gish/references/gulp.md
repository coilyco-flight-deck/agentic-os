...
`gish` lands only the current working tree. `gulp` adds the three classes `gish` leaves behind - **stashes, other branches, other worktrees** - and drains every repo across the fleet, so nothing stays parked.
...
0. **Fast-forward first** - `git pull --ff-only` from the canonical remote so local main absorbs what is already there. `--ff-only` is deliberate: never a merge, rebase, or invented history. A repo that cannot ff is not an error - **dirty** falls to step 1, **no-upstream / detached** skips with a report, **diverged** skips as "needs manual merge."
...
3. **All branches** - `git push <canonical> --all` so every local branch reaches canonical as its own branch. Repo-global - push once, not per worktree. Divergent branches are **preserved**, never force-collapsed onto main. gulp never `--force`.
...
Resolve as **"the live Forgejo pushurl if one exists, else the GitHub `origin`"**. If the grep is empty or the Forgejo push 404s, land `HEAD:main` (step 1) and `--all` (step 3) to GitHub `origin`. **Never skip a repo for an empty grep** - empty means "land to GitHub," not "undrainable." A 404ing `.github.git` remote is mis-wired, not a reason to park - drain to GitHub and flag the stale remote.

## Report

One block per repo, then a roll-up. Per repo: ff'd / up-to-date / couldn't-ff (with reason), issue #N, commit subjects, stashes landed / skipped, branches pushed, worktrees swept. **Flag every "needs manual merge" stash, every divergent branch, and every couldn't-ff repo** so nothing silently goes unmerged or stays behind.

## Confirm vs run

`gulp` is pre-authorized - it only pushes to Kai's own canonical mains (Forgejo, or GitHub for the profile repos) - so run it across many repos without a permission loop. **Surface, never force, when:** a stash apply conflicts, a branch cannot push non-destructively, a hook fails (fix the cause, never `--no-verify`), or a diff looks like work Kai clearly did not mean to land. Divergent branches go up via `push --all` as branches, never force-merged onto main.
