...
0. **Fast-forward first** - `git pull --ff-only` from the canonical remote so local main absorbs what's there. `--ff-only` is deliberate: never a merge, rebase, or invented history. Cannot ff is not an error - **dirty** falls to step 1, **no-upstream/detached** is reported, **diverged** flags "needs manual merge."
...
3. **All branches - merge, delete, keep the rest.** With main ff'd to canonical (step 0), merge each local branch (other than main) into main. **Clean or already-merged:** keep it, push main once after, then delete the branch. **Conflict:** `git merge --abort`, leave the branch, push it to canonical as its own branch, flag "needs manual merge." Gate each delete on `git merge-base --is-ancestor <branch> <canonical-main>`, then `git branch -D` (`-d` wrongly refuses a branch that is in main but ahead of its stale upstream). A worktree-checked-out branch cannot be deleted - prune the worktree or flag it. Never force-merge.
...
Resolve as **"the live Forgejo pushurl if one exists, else the GitHub `origin`"**. If the grep is empty or the Forgejo push 404s, land steps 1 and 3 to GitHub `origin`. **Never skip a repo for an empty grep** - empty means "land to GitHub," not "undrainable." A 404ing `.github.git` remote is mis-wired - drain to GitHub, flag it.
