...
Detail for [tooling-scout-displacement](../SKILL.md). The skill is two moves -
...
cadence, maintainer count, license) and run the supply-chain-audit skill - a young,
low-star tool is a real dependency risk. Land an adoption as one issue + one commit (the
migration). A subsystem that is the repo's whole reason for being usually stays custom.
