...
Reads mount release state (`list`, `status`, `history`, `get`) and the two offline renderers (`template`, `lint`, `show`). Writes mount the friendly deploy surface (`upgrade`, `install`, `rollback`); `upgrade --install` is how every rollout in deploy actually invokes it. Release destruction stays unexposed - `uninstall` and its `delete` alias are the irreversible pair, and `repo`, `registry`, `push`, `plugin`, `package`, and `create` are denied because each one changes where a chart comes from or runs code from a chart, which is a supply-chain decision rather than an operator verb. See the tooling-aosguard skill.
