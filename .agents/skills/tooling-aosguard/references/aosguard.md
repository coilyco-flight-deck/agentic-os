...
The snapshot carries Forgejo, AWS, kubectl, helm, Tailscale, Actions, Netlify, and runner-token leaves, and excludes Ward role policy. AWS SSM permits single reads, SecureString-only writes, and named deletions. Three denies that guarded this surface are **deliberately absent** since 2026-09-02, Kai's call rather than collateral from the same edit: a `*/prod/*` and `*secret*` deny on `get-parameter`, a `*tfstate*` and `*-backup*` deny on `s3 ls`, and the `file:///*` value rule on `put-parameter`. The last one is worth naming twice, because SSM doctrine still says to pass `--value file:///...` so a secret never enters argv: that is now convention rather than enforcement, live since aos-v0.295.0. `put-parameter` gained `when type matches SecureString`, which fails closed on an absent flag and is the surface's one mandatory-value rule. Actions lives at `aosguard ops actions` so its exec transport does not shadow `aosguard ops forgejo`, and carries only the log bridge. The sibling [`forgejo-storage measure` bridge](forgejo-ops.md) uses fixed `kubectl exec` operations from an embedded script invoked by absolute path.
