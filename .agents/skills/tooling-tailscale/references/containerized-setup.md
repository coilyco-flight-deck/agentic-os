...
This is the standing, shared box. It runs once with `restart: unless-stopped`, and the infra sibling (the `tailscale-proxy` ansible role in `coilyco/infrastructure`) converges it, not ward. The compose lives at `ansible/roles/tailscale-proxy/files/compose.yaml` and is authoritative. This excerpt focuses on its Tailscale service.
