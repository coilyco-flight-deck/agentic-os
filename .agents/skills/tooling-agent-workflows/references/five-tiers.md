...
1. **Description** - match time. Skill frontmatter `description:`. Aliases and triggers. The only surface doing fuzzy-matching work. Eager-loaded into every turn, so it stays under the validator's `max_description_bytes` cap (public default 500, agentic-os-kai 200; see the skill-discipline docs in agentic-os).
