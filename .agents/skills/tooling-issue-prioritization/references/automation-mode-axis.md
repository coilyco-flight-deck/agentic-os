...
Unlike the tier, the mode cannot come from a percentile cut - it is a per-issue judgment made during the triage pass (a `warded` triage run). For each issue (P0 included) the pass classifies it into one mode plus a confidence; only a high-confidence `headless`/`interactive` promotes the issue out of human-gated, everything else fails closed to `consult`.

## The dispatch gate

The enforcing half is a ceiling gate at the shared dispatch chokepoint: a surface runs an issue only when `surface <= mode` on the order `headless > interactive > consult`. It landed in agentic-os#246 (Phase 2): the gate lives in `cli-guard`'s `resolveDispatchIssue`, the chokepoint every surface flows through, reading the issue's mode label off `Issue.Labels` (populated by ward's forgejo and GitHub fetchers). The autonomous surfaces (`headless`, `cascade`, `ci`) gate at the headless ceiling, `interactive` at interactive, `consult` at consult. Unlabeled fails closed to consult. The three mode org-labels exist at org scope alongside P0-P4.
