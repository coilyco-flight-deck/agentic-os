...
Every skill is a peer directory directly under `.agents/skills/`. Do **not** nest sub-skills inside another skill's directory (e.g. `meta-skill/sub-skill/SKILL.md`). Nested-skill discovery is poorly supported by the harness, and the skill mount only symlinks top-level skill dirs into the harness skills dir (`~/.claude/skills/<name>`, `~/.codex/skills/<name>`). Anything below the top level is invisible to the loader.
