...
The **Goose** harness (she/her) - Block's open-source agent (`goose` CLI plus desktop app), built on MCP extensions and recipes. Harness axis, orthogonal to the `models-*` capability axis: it floats over many providers, so the active model is the ceiling, not "Goose". Binds to a cloud model or a local Ollama quant.
