...
7. **Spawn.**
   ```bash
   ward agent claude work coilysiren/<repo>#<N> --new-tab
   ```
   Always the interactive `work` surface for sidequests - Kai's eyes are on the spawned session. `--new-tab` opens it in its own Warp tab; the agent runs containerized (fresh clone, reaper-backed).
