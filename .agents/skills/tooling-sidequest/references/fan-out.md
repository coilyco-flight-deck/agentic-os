...
5. **Spawn the unblocked children.** `ward agent claude work <ref> --new-tab` for every child whose dependencies are already satisfied - all of them if fully parallel. Children blocked on an earlier child stay filed and linked but are NOT spawned yet. Note them in chat as blocked-on-`#N`.
