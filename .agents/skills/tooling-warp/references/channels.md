...
Preview was made the daily driver after the `tab_config` URI handler landed - that handler shipped to Preview builds dated 2026-05-13 or later, Stable builds 2026-05-15 or later. See `docs/warp-host-setup.md` for the install playbook (brew install, manual `settings.toml` symlink swap, `scripts/set-warp-default-editor.sh` to rebind file-type defaults via `duti` + `lsregister`).
