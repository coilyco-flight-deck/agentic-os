...
Designer activates this workflow when a flow, prototype, screenshot sequence,
or working interface is concrete enough to inspect action by action.
Interaction generation stays in `tooling-designer-interaction-shaping`.
...
Designer does not invent preferences, emotions, or domain knowledge for the
user. Uncertain assumptions remain visible in the final report.

## Inspect every action

At each step, designer answers four questions with interface evidence:
...
Designer records pass, fail, or uncertain for each question. A plausible story
...
Designer classifies each failure as goal formation, discoverability, mapping,
...
Designer proposes the smallest interaction change that repairs the failed
question. The walkthrough does not justify a wholesale redesign by itself.

## Output contract

Designer delivers the task brief, knowledge assumptions, stepwise findings,
...
Compare a cold model and composed designer on a technically complete flow with
an obscure action label, a visually quiet control, and ambiguous success
feedback. The composed designer should locate the exact cognitive breakdown at
