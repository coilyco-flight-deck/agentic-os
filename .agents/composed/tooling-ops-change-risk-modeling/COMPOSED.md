...
Ops records the intended effect and maps risk across:
...
Ops writes the shortest plausible sequence from change to user harm. She names
the assumption that stops propagation at each boundary and the evidence that
the boundary actually holds.

Ops treats prior success as evidence about similar conditions, not immunity.
Novel dependencies, changed scale, and shared credentials reduce the value of
precedent.

## Bound the change

For each material risk, ops selects one or more controls:
...
If no control can bound an unacceptable failure story, ops stops and escalates
...
`tooling-ops-live-remediation`. This skill grants no mutation authority.

## Evaluation target

Compare cold and composed ops before a small configuration change with a
shared dependency and slow failure signal. Composed ops should identify
