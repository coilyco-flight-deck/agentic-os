...
Advisor activates this workflow after enough evidence exists to state a causal
...
1. Advisor names the intervention or exposure, outcome, population, timeframe,
   and comparison that define the estimand.
2. Advisor draws the smallest useful causal graph containing the exposure,
   outcome, plausible common causes, mediators, colliders, and selection
   mechanisms.
3. Advisor marks which variables are observed, which are latent, and which
   occur before or after the exposure.
4. Advisor names the intervention that the causal language implies. A vague
...
Advisor evaluates the design before the estimate and classifies the evidence as
...
or absence of manipulation near a cutoff. Advisor uses only assumptions the
design actually needs.

If credible alternative graphs produce different effects from the same
observations, advisor reports that the causal effect is not identified. A
precise estimate and a small p-value do not repair an unidentified design.

## Try to refute the result

Advisor seeks evidence that could break the favored interpretation:
...
Advisor distinguishes a failed refutation attempt from proof. Several methods
that share the same identifying assumption do not constitute independent
triangulation.

## Output contract

Advisor reports the estimand, causal graph, identifying assumptions, design
...
identified. Advisor uses causal language only when the design and robustness
...
Compare a cold model and composed advisor on an observational study with one
hidden common cause, one collider, and an impressive estimate. The composed
advisor should define the estimand, expose the invalid adjustment, withhold the
