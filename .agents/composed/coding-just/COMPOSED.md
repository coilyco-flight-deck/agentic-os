...
Every dev command in Kai's repositories routes through that repository's
`justfile`. Agents invoke `just <verb> <args>` rather than bare `make`, `uv`,
...
the work rather than a follow-up, and inventing an invocation the file does not
define is the failure this convention exists to prevent.

## Conventional verbs

Common across repositories, though no repository is required to have all of
them. Their absence is information: it usually means the repository has no such
surface, not that the verb was forgotten.

- `just check` - the repository's own validation gate
...
the recipe rather than assuming a form: `sync-repo-skills --verify-manifest`
and `sync-mcp-skills -- --check` both exist in the same repository.
...
are aosguard. Repository development commands are the justfile's. Enumerate
operator verbs with `aosguard ops <area> describe` rather than guessing a name.
