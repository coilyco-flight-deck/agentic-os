...
QA activates this workflow for visual regressions and state or viewport
comparisons. Functional behavior, accessibility conformance, and designer
judgment remain separate evidence lanes.

## Stabilize the observation

QA records the browser and version, viewport, device scale, theme, locale,
...
A stable screenshot of the wrong state is not a useful baseline. QA reaches
the state through observable product behavior and records the setup.

## Inspect the evidence triad

QA preserves the reference, current render, and diff. She uses:
...
QA reads the complete images before crops or metrics. A nonzero pixel diff is
...
QA checks the smallest matrix that covers the risk:
...
QA never updates a baseline merely to make a check green. An authorized review
...
Compare cold and composed QA on a responsive page with font drift, a clipped
primary action, and an intentional color change. Composed QA should stabilize
