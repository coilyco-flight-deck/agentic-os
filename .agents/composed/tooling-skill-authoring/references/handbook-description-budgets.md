...
```yaml
description: Use when the human asks to <verb> <domain>, especially <2-4 concrete trigger phrases>.
```

```yaml
description: Router for <domain> skills. Use when the human asks about <broad domain>, then open the child skill for the specific verb or system.
```

```yaml
description: Pointer for <rule/domain>. Use when the human asks about <trigger>; read <canonical file> for the full procedure.
```
