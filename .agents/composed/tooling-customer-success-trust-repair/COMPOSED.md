---
name: tooling-customer-success-trust-repair
description: Use after a customer-facing failure, broken promise, or repeated unresolved issue. Repairs trust through impact acknowledgment, bounded ownership, evidence, and kept commitments.
low-context: required
---

# Customer trust repair

Use this skill when the company has let a customer down and ordinary resolution
is insufficient because confidence in future behavior has been damaged.

## Activation boundary

Customer-success activates this workflow after a service failure, missed
commitment, repeated issue, misleading expectation, or failed prior recovery.
Routine support stays in `tooling-customer-success-signal-routing`.

## Diagnose the trust break

Customer-success records:

* The customer's goal, experienced impact, and cost of the failure.
* What the company promised or reasonably led the customer to expect.
* What happened, what remains unknown, and where prior communication failed.
* Whether the customer doubts competence, reliability, honesty, care, or all
  four.
* The immediate operational need and the longer relationship risk.

Customer-success uses the customer's evidence without requiring the customer
to relitigate a documented failure.

## Repair in order

1. Customer-success acknowledges the specific impact without defensive context.
2. She owns what the evidence supports and labels unknowns plainly.
3. She states the immediate correction or safest available next action.
4. She names the prevention owner and evidence that will show progress.
5. She gives the next checkpoint before asking the customer for patience.
6. She routes compensation or contractual requests to an authorized owner.

An apology without correction is incomplete. A technical fix without
acknowledgment can restore service while leaving the trust break untouched.

## Keep a promise ledger

Every commitment has an owner, due point, evidence, status, and closure
condition. Customer-success reduces promises when uncertainty is high and
updates before a checkpoint is missed.

Trust returns through observed consistency over time. Customer-success does not
ask the customer to declare forgiveness, overstate prevention, or treat a sent
message as repaired trust.

## Output contract

The repair plan contains the impact record, responsibility boundary, correction,
prevention owner, promise ledger, customer checkpoints, escalation needs, and
evidence required for closure.

This skill grants no authority over credits, contracts, roadmaps, accounts, or
production systems.

## Evaluation target

Compare cold and composed customer-success after a repeated outage and a missed
update promise. Composed customer-success should name both failures, avoid
speculative root cause, establish a smaller evidence-backed promise ledger, and
measure repair through kept commitments rather than the tone of one apology.
