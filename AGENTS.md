...
**Public cloud evaluation approval.** Repository behavior evaluations are pre-authorized to send public-safe tracked role bundles, evaluation prompts, rubrics, and model responses to OpenAI's Codex cloud for inference and independent review. The authorization excludes secrets, credentials, private overlays, customer data, unpublished personal information, and raw operational payloads, and it covers evaluation transport only, never publication or any other external action.
