...
def _make_stubs(root: Path) -> tuple[Path, Path, Path, Path, Path]:
    ...
    _write_executable(
        ...
        textwrap.dedent(
            ...
            set -euo pipefail
            printf '%s\\n' "$*" >> {ward_log!s}
            if [[ "${{1:-}}" == ops && "${{2:-}}" == aws && "${{3:-}}" == sts && "${{4:-}}" == get-caller-identity ]]; then
            ...
            echo "unexpected ward invocation: $*" >&2
            exit 1
            """
        ),
    )
    ...
    return bin_dir, ward_log, gpg_log, import_log, passphrase_log


def _run(root: Path, *args: str, **env_overrides: str) -> subprocess.CompletedProcess[str]:
    bin_dir, ward_log, gpg_log, import_log, passphrase_log = _make_stubs(root)
...
def test_passthrough_non_signing_invocations(tmp_path: Path) -> None:
    ...
    assert (tmp_path / "ward.log").read_text(encoding="utf-8") == ""
...
def test_bootstraps_shared_secret_key_when_missing(tmp_path: Path) -> None:
    ...
    assert result.returncode == 0
    assert (tmp_path / "ward.log").read_text(encoding="utf-8").splitlines() == [
        "ops aws sts get-caller-identity",
        "ops aws ssm get-parameter --name /coilysiren/gpg-secret-key --with-decryption --query Parameter.Value --output text",
        "ops aws ssm get-parameter --name /coilysiren/gpg-passphrase --with-decryption --query Parameter.Value --output text",
    ]
